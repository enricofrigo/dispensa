...
public interface OpenFoodFactCacheDao {
    ...
    @Query("DELETE FROM openfoodfact_cache WHERE barcode IN (SELECT barcode FROM openfoodfact_cache ORDER BY timestamp_ms ASC, barcode ASC LIMIT :amountToDelete)")
    ...
    @Query("SELECT image_local_path FROM openfoodfact_cache ORDER BY timestamp_ms ASC, barcode ASC LIMIT :amountToDelete")
